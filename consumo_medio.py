...
def main():
    ...
    # Na base valores, a coluna de data pode ser 'EMISSÃO' (prioritário) ou 'DATA'
    col_data_val = next((c for c in df_val.columns if 'EMISSÃO' in c), None)
    if not col_data_val:
    ...
    with st.sidebar:
        ...
        # Remover placas inválidas do filtro
        placas = [p for p in placas if p not in ('', '-', 'CORRECAO')]
    ...
    with tab1:
        ...
        df_kpi = pd.DataFrame({
            ...
            'Interno': [litros_int_saida, valor_int]
