...
def main():
    ...
    # --- FILTRO DE COMBUSTÍVEL DINÂMICO COM BOTÃO SELECIONAR TODOS / LIMPAR ---
    combustivel_col = next((col for col in df_ext.columns if 'DESCRI' in col), None)
    combustiveis_escolhidos = []
    if combustivel_col:
        ...
        st.markdown("### 🔍 Filtro de Tipo de Combustível (Externo)")
        cols_filt = st.columns([0.7, 0.15, 0.15])
        with cols_filt[0]:
            combustiveis_escolhidos = st.multiselect(
                'Selecione um ou mais combustíveis',
                options=tipos_disponiveis,
                default=tipos_disponiveis,
                help='Selecione um ou mais tipos de combustível para filtrar'
            )
        with cols_filt[1]:
            if st.button('Selecionar Todos'):
                combustiveis_escolhidos = tipos_disponiveis
        with cols_filt[2]:
            if st.button('Limpar Seleção'):
                combustiveis_escolhidos = []

        if combustiveis_escolhidos:
            df_ext = df_ext[df_ext[combustivel_col].isin(combustiveis_escolhidos)]
        else:
            # Se não selecionou nada, exibe aviso e não filtra
    ...
    with tab1:
        ...
        # Gráfico Resumo - barras lado a lado com % na barra e cores fixas
        df_kpi = pd.DataFrame({
        ...
        # Acrescentar percentual para a métrica Litros no texto do gráfico
        def format_text(row):
            if row['Métrica'] == 'Litros':
                perc = perc_ext if row['Tipo']=='Externo' else perc_int
                return f"{row['Valor']:,.0f} L\n({perc:.1f}%)"
            else:
                return f"R$ {row['Valor']:,.2f}"

        df_kpi['Texto'] = df_kpi.apply(format_text, axis=1)

        colors = {'Externo': '#1f77b4', 'Interno': '#2ca02c'}
        ...
        fig.update_layout(title='Comparativo Externo vs Interno',
                          title_font_size=22,
