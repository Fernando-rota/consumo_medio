...
def main():
    ...
    with tab4:
        ...
        st.plotly_chart(px.line(df_int_agg, x='DATA', y='QTDE_LITROS', markers=True, title='Litros Consumidos (Interno)'), use_container_width=True)
        st.plotly_chart(px.line(df_val_agg, x='DATA', y='VALOR', markers=True, title='Custo Total (Interno)'), use_container_width=True)
