...
def main():
    ...
    up1 = col1.file_uploader('Externo', type=['csv','xlsx'], key='e')
    up2 = col2.file_uploader('Interno', type=['csv','xlsx'], key='i')
    up3 = col3.file_uploader('Valor Int.', type=['csv','xlsx'], key='v')
    ...
    col3 = next((c for c in base3.columns if 'dt' in c.lower() or 'data' in c.lower()), None)
    if not col3:
        st.error('Data não encontrada em Valor Int.')
        return
    base3['data'] = pd.to_datetime(base3[col3], dayfirst=True, errors='coerce')
    ...
    with f1:
        sd = st.date_input('De', value=pd.to_datetime('2025-01-01'))
    with f2:
        ed = st.date_input('Até', value=pd.to_datetime('2025-12-31'))
    with f3:
    ...
    mask1 = (base1['data'].dt.date >= sd) & (base1['data'].dt.date <= ed)
    mask2 = (base2['data'].dt.date >= sd) & (base2['data'].dt.date <= ed)
    mask3 = (base3['data'].dt.date >= sd) & (base3['data'].dt.date <= ed)
    base1 = base1[mask1]
    base2 = base2[mask2]
    base3 = base3[mask3]
    if filt != 'Todos':
        base1 = base1[base1['DESCRIÇÃO DO ABASTECIMENTO'] == filt]
